...
const void SyncedMemory::cpu_data(){
    to_cpu(); //decide whether to actually copy the data according to the current state.
    return (const void*)cpu_ptr;
}

const void SyncedMemory::gpu_ptr(){
    to_gpu();
...
void* SyncedMemory::mutable_cpu_data(){
    to_cpu();
    //restart the state mechine when at state "SYNCED"
    head_ = HEAD_AT_CPU;
    return cpu_ptr;
}
...
void SyncedMemory::set_cpu_data(void *data){
    ...
    cpu_ptr = data;
    head_ = HEAD_AT_CPU; //synced for next call
    own_cpu_data = false;
}
